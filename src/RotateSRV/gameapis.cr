  ...
  class GameAPIs
    ...
    def self.add_domains_to_database(path : String) : Bool
      ...
      domains.each do |domain|
        ...
        begin
          response = Cossack.get("https://gameapis.io/api/mc/extra/blockedservers/check/#{domain.downcase}")
        rescue ex
    ...
    def self.check_database(path : String) : Bool
      ...
      domains.each do |domain|
        ...
        begin
          response = Cossack.get("gameapis.io/api/mc/extra/blockedservers/check/#{domain.downcase}")
        rescue ex
      ...
      begin
        current_response = Cossack.get("gameapis.io/api/mc/extra/blockedservers/check/#{current[0].downcase}")
      rescue ex
        puts "Error when contacting GameAPIs for checking if a domain is blacklisted. (#{ex.message})"
        return false
      end
      ...
      current_response_json["#{current[0]}"].as_a.each do |check|
